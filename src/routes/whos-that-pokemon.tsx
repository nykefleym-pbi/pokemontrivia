...
function sameTypes(sel: PokeType[], actual: PokeType[]): boolean {
  if (sel.length !== actual.length) return false;
  return [...sel].sort().join(",") === [...actual].sort().join(",");
}
...
function makeRound(): Round {
  ...
  return {
    monId: mon.id, name: mon.name, types: mon.types,
    mode: Math.random() < 0.5 ? "1A" : "1B",
...
function WhosThatPokemon() {
  ...
  useEffect(() => {
    ...
    if (Math.floor(Date.now() / HOUR) === whosThatHourKey) {
      setLocked(true);
    } else {
      setRound(makeRound());
      consumeWhosThat();
  ...
  useEffect(() => {
    if (phase === "correct" && round && !claimedRef.current) {
      ...
      recordPokedexCapture(round.monId, round.isShiny);
    }
  }, [phase, round, addXp, grantItem, recordPokedexCapture]);
  ...
  function submit1B() {
    if (!round || selTypes.length === 0) return;
    ...
    return (
      <div className="flex h-full w-full flex-col overflow-y-auto bg-poke-cream px-5 pb-8 pt-10">
        <div className="relative flex flex-col items-center">
          ...
          {round.isShiny && (
            <div className="z-10 mb-1 rounded-full bg-poke-yellow px-3 py-1 font-pixel text-[10px] text-poke-dark shadow-card">✨ SHINY!</div>
          )}
          <PokemonSprite id={round.monId} shiny={round.isShiny} alt={round.name}
            className={`relative z-10 h-44 w-44 [image-rendering:pixelated] ${round.isShiny ? "drop-shadow-[0_0_14px_rgba(245,197,24,0.85)]" : ""}`} />
          <h1 className="z-10 mt-2 text-3xl font-extrabold text-poke-dark">It's {round.name}!</h1>
        </div>
        <div className="mt-6 space-y-3">
          ...
          <Row sub="You got" title={round.rewardName}
            icon={<div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-primary/10"><img src={round.rewardIcon} alt={round.rewardName} className="h-7 w-7 [image-rendering:pixelated]" /></div>} />
    ...
    <div className="flex h-full w-full flex-col overflow-y-auto bg-poke-cream px-5 pb-8 pt-6">
      ...
      {round.mode === "1A" ? (
        <div className="mt-auto pt-8">
          <input value={guess} onChange={(e) => setGuess(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") submit1A(); }}
            placeholder="Type the Pokémon's name…" autoFocus
          ...
          <button onClick={submit1A} className="mt-3 w-full rounded-full bg-primary py-4 font-pixel text-base tracking-wide text-primary-foreground shadow-card active:scale-[0.98]">SUBMIT</button>
        </div>
      ) : (
        <div className="mt-5">
          ...
          <button onClick={submit1B} className="mt-5 w-full rounded-full bg-primary py-4 font-pixel text-base tracking-wide text-primary-foreground shadow-card active:scale-[0.98]">SUBMIT</button>
        </div>
      )}
    </div>
  );
