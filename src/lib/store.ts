...
/**
 * The persisted subset of GameState — shared by the zustand `persist`
 * middleware (localStorage) and store-sync.ts (server save-sync push), so the
 * two never drift into serializing different shapes of "the save".
 */
export function buildSavePayload(s: GameState) {
  return {
...
export const useGameStore = create<GameState>()(
  persist(
    (set, get, store) => ({
      ...
      serverSaveVersion: 0,
      setServerSaveVersion: (v) => set({ serverSaveVersion: v }),
...

    {
      ...
      partialize: (s) => ({
        ...buildSavePayload(s),
        // Sync bookkeeping: not part of "the save" itself, but must survive
        // reloads so store-sync.ts knows the right baseVersion to push next.
        serverSaveVersion: s.serverSaveVersion,
      }),
