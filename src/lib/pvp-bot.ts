...
/**
 * Pure, testable brain for the Training-vs-Bot mode. Rolled server-side (inside
 * `pvp-live-resolve-turn`, via `engine/pvp-live-turn.ts`'s `rollBotTurn`) from
 * the match's persisted `guest_bot_profile` — the human's device never decides
 * or reports the bot's move. Everything here is deterministic given an injected
 * `rng` (defaults to Math.random). Mirrors the style of pvp-combat.ts — flat
 * helpers, no state.
 */
