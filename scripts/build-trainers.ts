...
function shouldSkip(filename: string): boolean {
  ...
  if (
    /(^|_)(Alt|EX|Sygna|Holiday|Palentines|Anniversary|Special|Costume|Champion|Kimono|Academy)(_|$)/i.test(
      base,
    )
  )
    return true;
...
async function scrapeCategory(startUrl: string): Promise<Raw[]> {
  ...
  while (url && page < 20) {
    ...
    const re =
      /<img alt="" src="(https:\/\/archives\.bulbagarden\.net\/media\/upload\/[^"]+\.png)"[^>]*>[\s\S]*?title="File:([^"]+\.png)"/g;
    let m: RegExpExecArray | null;
    while ((m = re.exec(html))) {
      const fullUrl = m[1]
        .replace("/media/upload/thumb/", "/media/upload/")
        .replace(/\/\d+px-[^/]+$/, "");
      const filename = m[2];
...
async function main() {
  ...
  // Download sprite to public/trainers/avatar/{id}.png so the app references local files.
  if (!existsSync(AVATAR_DIR)) mkdirSync(AVATAR_DIR, { recursive: true });
  let downloaded = 0;
  for (const t of list) {
    const dest = join(AVATAR_DIR, `${t.id}.png`);
    if (!existsSync(dest) || statSync(dest).size === 0) {
      const r = await fetch(t.url, {
        headers: { "User-Agent": "Mozilla/5.0 trainer-roster-builder" },
      });
      if (!r.ok) {
        console.warn(`skip ${t.id}: HTTP ${r.status}`);
        continue;
      }
