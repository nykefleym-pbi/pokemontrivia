-- Phase 5: weather-conflict resolution for Kyogre (382) / Groudon (383) /
-- Rayquaza (384), now that both partner ids are known (Phase 1).
--
--   * Rayquaza on EITHER side negates both Kyogre's and Groudon's standing
--     weather-driven stat effects for the whole match (Air Lock / Delta Stream
--     lore). The server refuses to apply a 382/383 post_answer weather effect
--     whenever a 384 is present -- no charge, no bump.
--   * Otherwise, "latest (re-)established weather wins": each successful
--     382/383 weather application records that side as `weather_owner`. A null
--     owner (turn-1 tie) resolves in the HOST's favour in the pure client
--     resolver. NOTE: the shipped stage system stores bumps that cannot be
--     auto-un-applied, so mechanical suppression of the LOSER's already-applied
--     weather stages is not enforced here -- `weather_owner` is the shared
--     source of truth the client uses to attribute the current weather and is
--     ready for a future per-source stage-reset. Flagged for the product owner.
--   * Mirror matchups (both 382 or both 383) record ownership independently and
--     get no special interaction.
--
-- The apply_pvp_signature_effect body below is reproduced from
-- 20260705170000_pvp_signature_suppress.sql with the weather negation gate and
-- weather_owner maintenance added.

alter table public.pvp_live_matches
  add column if not exists weather_owner text;
